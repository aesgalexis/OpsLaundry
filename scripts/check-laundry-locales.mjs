...
for (const lang of LANGS) {
  for (const page of Object.keys(ROUTES[lang])) {
    ...
    const pageNavScripts = bodyNodes.filter((node) => node.tagName === "script" && attribute(node, "src") === "/ls_page-nav.js");
    ...
    const pageScripts = bodyNodes.filter((node) => node.tagName === "script" && attribute(node, "src") === "/ls_page.js");
    if (pageScripts.length !== 1) failures.push(`${route}: expected one direct-navigation script, found ${pageScripts.length}.`);
    const topbarScripts = bodyNodes.filter((node) => node.tagName === "script" && attribute(node, "src") === "/ls_top-bar.js");
    ...
    if (page === "contact") {
      ...
      const contactScripts = bodyNodes.filter((node) => node.tagName === "script" && attribute(node, "src") === "/contact.js");
      if (!hasClass(heading, "contact-page-title--visually-hidden")) failures.push(`${route}: contact h1 must remain visually hidden.`);
      if (!card || !form || !formNodes.some((node) => hasClass(node, "form-grid"))) failures.push(`${route}: incomplete original contact form structure.`);
      if (attribute(textarea, "rows") !== "6" || !hasClass(textarea, "field")) failures.push(`${route}: incorrect contact message field.`);
      if (attribute(subject, "type") !== "hidden" || attribute(subject, "value") !== "laundryservices") failures.push(`${route}: contact subject must stay fixed to Laundry Services.`);
...
const pageSource = await readFile("ls_page.js", "utf8");
if (!pageSource.includes('["/",')) failures.push("ls_page.js: missing direct route for root.");
