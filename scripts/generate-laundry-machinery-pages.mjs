...
const loadRuntimeConfig = async (root) => {
  ...
  const match = source.match(/window\.__UNATOMO_CONFIG__\s*=\s*(\{[\s\S]*?\})\s*;/u);
...
const generateDetailPage = (template, machine, lang, copy) => {
  ...
  html = html.replace('<script type="module" src="/ls_maquinaria.js"></script>', '<script type="module" src="/ls_machine-detail.js"></script>');
  return html;
};
...
export const generateLaundryMachineryPages = async ({root, dist}) => {
  let config;
  try {
    config = await loadRuntimeConfig(root);
  } catch {
    console.warn("Laundry snapshot omitido: no se pudo leer runtime-config.js.");
    return {generated: 0, machines: 0};
  }
  if (!config.FIREBASE_PROJECT_ID || /your_project/iu.test(config.FIREBASE_PROJECT_ID)) {
    console.warn("Laundry snapshot omitido: FIREBASE_PROJECT_ID no está configurado.");
    return {generated: 0, machines: 0};
  }
  let machines;
  try {
    ...
    console.warn(`Laundry snapshot omitido sin interrumpir el build: ${error.message}`);
    return {generated: 0, machines: 0};
  }
  machines = sortMachines(machines.filter((machine) => safeMachineId(machine.id)));
  if (!machines.length) {
    console.warn("Laundry snapshot omitido: Firestore no devolvió máquinas públicas visibles.");
    return {generated: 0, machines: 0};
  }
  let generated = 0;
  ...
  console.log(`Laundry snapshot: ${machines.length} máquinas visibles y ${generated} fichas HTML.`);
