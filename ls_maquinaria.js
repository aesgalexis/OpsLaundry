import { subscribeMachines } from "/ls_maquinaria/agregador/ls_machine-public-store.js";

const LANGS = ["es", "en", "it", "el"];
const PAGE_SIZE = 20;
const CONTACT_LABELS = {es: "Contactar", en: "Contact", it: "Contatta", el: "Επικοινωνία"};
...
window.setTimeout(() => {
  ...
  if (!currentMachines.length && (!hasPrerenderedRows || requestedMachineId)) {
...
if (copies.length) {
  ...
  const buildMachineHref = (machine) => {
    ...
    const base = window.location.pathname.endsWith("/")
      ...
      : `${window.location.pathname}/`;
    return document.body.dataset.staticMachinePages === "true"
      ? `${base}${id}/`
      : `${base}?machine=${id}`;
  };
  ...
  const renderMachineRows = (machine, lang) => {
    ...
    const galleryInitiallyOpen = Boolean(requestedMachineId && hasImages);
    const extrasText = extras.length ? ` · ${extras.join(" · ")}` : "";
    const visibilityText = isMachineAdmin && machine.visible === false ? " · Oculta" : "";
    const machineHref = buildMachineHref(machine);
    const infoHref = requestedMachineId ? buildContactHref(machine, lang) : machineHref;
  ...
  const getFilteredMachines = (copy, machines) => {
    if (requestedMachineId) {
      return machines.filter((machine) =>
        String(machine.id).toLowerCase() === requestedMachineId.toLowerCase());
    }
  ...
  import("/ls_maquinaria/agregador/firebase-config.js").then(({observeMachineAdmin, isAdminUser}) => {
    let loaded = false;
    observeMachineAdmin((user) => {
      ...
      stylesheet.href = "/ls_maquinaria/agregador/ls_machine-add.css";
      document.head.append(stylesheet);
      import("/ls_maquinaria/agregador/ls_machine-add.js");
      import("/submissions/admin-link.js");
    });
  }).catch(() => {});
}
